#!/usr/bin/env python
"""
Unified evaluation for base, SFT, and PPO weather model variants.

This module is the dedicated implementation for evaluation. The legacy
`evaluate_ppo_weather.py` script remains as a thin compatibility wrapper.
"""
...
from ppo_utils import resolve_path, save_json, write_jsonl
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--forbidden-generation-chars", default="")
...
def evaluate_model(
    ...
    for example in examples:
        prediction, latency = agent.generate_text(
            model=model,
            tokenizer=tokenizer,
        ...
        rows.append(
            {
                ...
                "json_valid": int(breakdown.get("json_valid", 0)),
    ...
    summary = {
        ...
        "json_valid_rate": round(statistics.mean(json_valid_values), 4) if json_valid_values else None,
        "recoverable_json_rate": round(statistics.mean(recoverable_values), 4) if recoverable_values else None,
...
def merge_prediction_rows(results: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row_index, base_row in enumerate(base_rows):
        ...
        for label in labels:
            ...
            merged[f"{label}_json_valid"] = source["json_valid"]
            merged[f"{label}_recoverable_json"] = source["recoverable_json"]
...
def main() -> None:
    ...
    base_tokenizer = agent.load_tokenizer(padding_side="left")
    ...
    for label, adapter_path in model_specs:
        ...
        model_examples = load_weather_prompt_examples(
            dataset_path=dataset_path,
            tokenizer=tokenizer,
            max_samples=args.max_samples,
            shuffle=args.shuffle,
        ...
        model, _resolved_load_mode, _compute_dtype = agent.load_model_variant(
            adapter_path=adapter_path,
            load_mode=load_mode,
            is_trainable=False,
        )
    ...
    report = {
        ...
        "metric_definitions": {
            ...
            "json_valid_rate": "Fraction of samples that parse directly as valid JSON objects.",
            "recoverable_json_rate": "Fraction of malformed samples where schema fields could still be recovered.",
    ...
    for label, payload in results.items():
        if label == "base":
            continue
        comparison = {}
        for metric_name in ("accuracy", "hallucination_rate", "a1_score", "exact_json_match", "json_valid_rate", "recoverable_json_rate", "avg_reward", "exact_match", "token_f1"):
